...
export async function generateStaticParams(): Promise<Params[]> {
  return mockBlogPosts.map((p) => ({ title: slugify(p.title) }));
}

/* -- SEO -------------------------------------------------------------- */
export async function generateMetadata({ params }: { params: Params }) {
  const post = mockBlogPosts.find((p) => slugify(p.title) === params.title);
  if (!post) return {};

  const image =
    post.image ??
    `https://images.unsplash.com/random/800x420?sig=${params.title}`;
...
export default function BlogDetailPage({ params }: { params: Params }) {
  ...
  const img =
    post.image ??
    `https://images.unsplash.com/random/1200x600?sig=${params.title}`;
